...
During lazy sweep, eden is swept one page at a time providing up to 408 object slots for re-use per page. By the time the lazy sweep completes, all unmarked slots in eden have been replaced by new objects.
...
Ruby 2.2 will likely expand the GC algorithm from two generations to three. In fact, 2.1 already includes a `RGENGC_THREEGEN` compile flag to enable the third generation. Additional bitmaps could also be used to implement a "mostly-moving" GC, by keeping track of objects which are allowed to move.
